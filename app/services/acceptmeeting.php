...
if (mysqli_num_rows($result)==1){
    ...
    if ($result && startsWith($mr_id,"mr_")){
        ...
        if (mysqli_num_rows($result)==1){
            ...
            if ($result){
                if($bounce){
                    $content = "<b>".$_SESSION['fname']." ".$_SESSION['lname']."</b> accepted your meeting request reshedule.";
                    $updatequery = "INSERT INTO notification(member_id, message, unshown, seen, action) VALUES ($target, \"$content\",1,0,\"ma_".$mr_id."\")";
                    $res = mysqli_query($conn,$updatequery);
                }else{
                    ...
                    $result = mysqli_query($conn,$updatequery);
                    $row = mysqli_fetch_assoc($result);
                    $content = "<b>".$row['first_name']." ".$row['last_name']."</b> accepted your meeting request.";
                    $updatequery = "INSERT INTO notification(member_id, message, unshown, seen, action) VALUES ($target, \"$content\",1,0,\"ma_".$mr_id."\")";
                    $res = mysqli_query($conn,$updatequery);
                }
                echo "success";
